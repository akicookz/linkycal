...
8. **MCP server** - `/api/mcp` Streamable HTTP endpoint (`worker/mcp/`) exposing bookings, availability, contacts, event types, forms, workflows, and analytics as MCP tools; authenticated with OAuth and bound to one administrator-approved project
