  ...
  return (
    <div className={`blog-cover${compact ? " blog-cover-card" : ""}`} aria-label={`${post.title} cover`}>
      ...
      <div className={`relative z-10 flex h-full flex-col justify-between ${compact ? "blog-cover-card-content" : "p-7 sm:p-10"}`}>
        ...
        <p className={compact ? "max-w-3xl text-balance font-heading text-xl font-semibold leading-[1.02] tracking-[-0.04em] text-white sm:text-2xl" : `max-w-3xl text-balance font-heading font-semibold leading-[0.98] tracking-[-0.045em] text-white ${post.title.length > 70 ? "text-3xl sm:text-5xl" : "text-4xl sm:text-6xl"}`}>{post.title}</p>
