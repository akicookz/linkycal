...
function authRequest(scopes: string[] = ["read", "write"]): AuthRequest {
  return {
    ...
    scope: scopes,
    state: "state-123",
    codeChallenge: VALID_PKCE_CHALLENGE,
...
describe("MCP OAuth authorization", function () {
  ...
  test("authorization requires code flow with an RFC 7636 S256 challenge", async function () {
    ...
    const cases: Array<{
      ...
      {
        label: "implicit response",
        request: { ...authRequest(), responseType: "token" },
        error: "authorization code flow",
      },
      {
        label: "missing challenge",
        request: { ...authRequest(), codeChallenge: undefined },
        error: "valid S256 PKCE challenge",
      },
      {
        label: "unsupported challenge method",
        request: { ...authRequest(), codeChallengeMethod: "S512" },
        error: "valid S256 PKCE challenge",
      },
      {
        label: "malformed challenge",
        request: { ...authRequest(), codeChallenge: "too-short" },
        error: "valid S256 PKCE challenge",
      },
    ];
  ...
  test("refresh tokens are enabled only for grants that include offline access", function () {
    const props = {
    ...
    expect(
      ...
      accessTokenProps: { scopes: ["read"] },
      refreshTokenTTL: 0,
    });

    const offlineResult = mcpTokenExchangeResult({
      grantType: "authorization_code",
      scope: ["read", "offline_access"],
      requestedScope: ["read"],
      props,
    });
    expect(offlineResult).not.toHaveProperty("refreshTokenTTL");

    const refreshResult = mcpTokenExchangeResult({
      grantType: "refresh_token",
      scope: ["read", "offline_access"],
      requestedScope: ["read"],
      props,
    });
    expect(refreshResult).not.toHaveProperty("refreshTokenTTL");
  });

  test("consent exposes every project where the current user is an administrator", async function () {
    const testDatabase = createTestDb();
    const oauth = new FakeOAuthHelpers();
    try {
      ...
      expect(context.projects).toEqual([
        ...
        { id: "oauth-free-project", name: "Free Project" },
      ]);
