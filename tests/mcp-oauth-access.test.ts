...
describe("MCP OAuth runtime access", function () {
  test("an active project administrator is authorized on every plan with or without a trusted LinkyCal Origin", async function () {
    for (const plan of ["free", "pro"] as const) {
      const testDatabase = createTestDb();
      try {
        const props = await seedAccess(testDatabase, { plan });
  ...
  test("revocation, lost administration, project mismatch, and untrusted Origin stop access", async function () {
    const cases: Array<{
      label: string;
      options: SeedOptions;
      origin?: string;
